...
- **【進行中】ローカル2フォルダの統合(2026-08-17着手)**: 現在、ローカルには以下2つのフォルダが並存しており、二重管理になっている。
  ...
  - **統合方針**: `analysis`フォルダ自体を`rz`リポジトリのgit管理下に置き、1フォルダに一本化する。
  - **注意点**: `analysis`フォルダにはWindowsタスクスケジューラ5タスクの実行パスが紐づいているため、フォルダ構成(パス)を変更する場合はタスク定義(実行ファイルのパス指定)への影響を必ず確認してから移行すること。安全に確認するため、ユーザーが対象5タスクの設定をXML形式でエクスポート済み(2026-08-17)。次回セッションでこのXMLを元に、パス変更の影響有無を確認したうえで移行作業を行う。
