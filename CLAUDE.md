...
@../CLAUDE.md
<!-- 上記は python/ 直下の業務領域マップ(analysis以外にどんな領域フォルダがあるか)を明示的にインポートしている。
     analysisは独立したgitリポジトリのため、python/CLAUDE.mdは通常の親ディレクトリ自動読み込みの対象外になりうるので、
     @インポートで確実に読み込む形にしている(2026-08-21)。 -->
...
マッチング(実提案)→企業担当◯→設定(本交渉設定)という、成約の前段階KPIを個人/T/課/全体の4粒度でPDCA管理しているスプレッドシート「【29-4Q】ロンザンPDCA管理」の構造(タブ構成、実績/予定の列ペアと3ルート(初期交渉/反響/他人マッチング)、個人別タブの人ブロック行構造、バックエンドデータ列の対応、2026-08-18に発覚・修正した日付範囲countifsの境界条件バグ)は [docs/spreadsheet-pdca-kanri-structure.md](docs/spreadsheet-pdca-kanri-structure.md) にまとめている。このシートで「特定の週・特定セルだけ数値が反映されない」といった相談が来た際は、まずこのファイルの6章(既知の不具合)を参照する。
...
- (2026-08-17解決)ローカル2フォルダの統合: `Z:\Users\suehara\Documents\python\analysis`(本来の作業フォルダ、token.pickle・credentials.json・.venv実在、Windowsタスクスケジューラ5タスクの実行パスもここ)を、[docs/local-folder-migration.md](docs/local-folder-migration.md)の手順通りin-place化(`analysis`フォルダ自体をgit管理下に変換)して完了。タスクスケジューラ・`.bat`のパスは無変更のまま、`run_shoki_seisansei.bat`の実行で動作確認済み。重複していた`Z:\Users\suehara\Documents\GitHub\rz`クローンは削除。以後、ブラウザ版Claude Codeが参照する本リポジトリと、ローカルの実行環境は同一の`analysis`フォルダになった。
